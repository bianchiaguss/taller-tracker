  ...
  return (
    <Modal open onClose={onClose} size="md">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div><label className="label">Nombre del cliente</label><input className="input" required value={form.nombre} onChange={set('nombre')} /></div>
          <div><label className="label">Fecha</label><input className="input" type="date" required value={form.fecha} onChange={set('fecha')} /></div>
          <div>
            <label className="label">Calificación</label>
            <StarPicker value={form.calificacion} onChange={v => setForm(p => ({ ...p, calificacion: v }))} />
          </div>
          <div><label className="label">Comentario</label><textarea className="textarea" rows={4} required value={form.comentario} onChange={set('comentario')} placeholder="Lo que dijo el cliente sobre el servicio…" /></div>
